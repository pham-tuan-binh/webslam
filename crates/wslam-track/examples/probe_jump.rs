//! Temporary diagnostic probe: the huge-jump / pure-rotation interruptions.
use wslam_core::*;
use wslam_track::*;
...
fn main() {
    ...
    for i in 0..25 {
        tracker.process(&frame(i, render(&sc, &truth_pose(i), &k, 1.0)), None);
    }
    println!("setup state {:?}", tracker.state());
    let jumped = Se3::new(
        truth_pose(25).rotation(),
        truth_pose(25).translation() + Vec3::new(2.0, 0.6, 0.0),
    );
    let out = tracker.process(&frame(25, render(&sc, &jumped, &k, 1.0)), None);
    println!("jump -> {:?} tracked {}", out.state, out.tracked_count);

    println!("=== pure rotation ===");
    let mut tracker = Tracker::new(TrackConfig::default(), k);
    for i in 0..40 {
        let pose = Se3::from_rotation(So3::exp(&Vec3::new(0.0, 0.006 * i as f64, 0.0)));
